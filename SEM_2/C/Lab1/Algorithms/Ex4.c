...
void fillArrWithRnd(int size, float arr[size]) {
    for (int i = 0; i < size; ++i)
        arr[i] = randV(LowerBound, UpperBound);
}

void printArray(int size, const float arr[size]) {
    for (int i = 0; i < size; ++i)
        printf("%f\t", arr[i]);
...
float sum(int size, const float arr[size], float *max, float *min, float *average, float *maxDis, float *minDis) {
    float sum = arr[0];
    *max = arr[0];
    *min = arr[0];
    *average = arr[0];
...
void revertBubbleSort(int size, float arr[size], int print) {
